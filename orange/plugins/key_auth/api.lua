...
local key_auth_db = require("orange.plugins.key_auth.key_auth_db")
...
API["/key_auth/enable"] = {
    POST = function(store)
        return function(req, res, next)
            ...
            local update_result = key_auth_db.replace_meta_value(store,"key_auth.enable", key_auth_enable )
			--[[
			store:update({
...
API["/key_auth/fetch_config"] = {
    ...
    GET = function(store)
        return function(req, res, next)
            ...
            -- 查找enable
            local enable, err1 = key_auth_db.select_meta_value(store,"key_auth.enable")
			--[[
			store:query({
            ...
            -- 查找rules
            local rules, err2 = key_auth_db.select_key_auth_rules(store)
			--[[
			store:query({
...
API["/key_auth/configs"] = {
    ...
    PUT = function(store)
        return function(req, res, next)
            ...
            -- 插入到mysql
            local insert_result = key_auth_db.insert_key_auth_rules(store,rule.id,rule)
			--[[
			store:insert({
    ...
    DELETE = function(store)
        return function(req, res, next)
            ...
            local delete_result = key_auth_db.delete_key_auth_rules(store,rule_id)
			--[[
			store:delete({
    ...
    POST = function(store)
        return function(req, res, next)
            ...
            local update_result = key_auth_db.update_key_auth_rules(store,rule.id,rule)
			--[[
			store:delete({
